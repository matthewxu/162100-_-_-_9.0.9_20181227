...
//注意:使用之前请去网上下载纯真IP数据库,并改名为 "ip.dat" 放到当前目录下即可.
//by 查询吧 www.query8.com
//*


$GLOBALS['IP_DATA']=(basename(dirname($_SERVER['PHP_SELF']))=='weather'?'':'readonly/weather/').'ip.dat';
...
class ipLocation{
  ...
  function __construct(){
    $this->fp=fopen($GLOBALS['IP_DATA'],'rb');   //二制方式打开
  ...
  function getIP(){
    ...
    }elseif(($_SERVER['REMOTE_ADDR']=str_replace('::1', '', $_SERVER['REMOTE_ADDR'])) && $_SERVER['REMOTE_ADDR']!='127.0.0.1'){ 
  ...
  */
  ...
  function getaddress($ip){
    ...
    //*过滤一些无用数据
    $address["area1"] = (!empty($address["area1"]) && !strstr($address["area1"],'本机') && !strstr($address["area1"],'局域网') && !strstr($address["area1"],'IANA') && !strstr($address["area1"],'CZ88')) ? $address["area1"] : '北京';
...
if ($_GET["action"]){
  ...
  if($_GET["action"]=="queryip"){
    $myobj=new ipLocation();
    $address=$myobj->getaddress($_GET["ip_url"]);
    $myobj=NULL;
